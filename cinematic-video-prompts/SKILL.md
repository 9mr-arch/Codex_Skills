...
description: 실사 영상 생성 프롬프트를 8개 브리프 질문으로 설계하고 작성하거나 수정한다. 스타일프레임의 무드와 시각 문법, 인물 참조, 시간순 숏, 행동, 카메라와 렌즈 질감, 전환, 음향, 연속성을 통합하고 각 프롬프트 작성 전에 한국어로 연출 내용을 설명한다. TVCF, 폰카, 드라마, 패션 영상 등 구체적인 촬영 연출이 필요한 영상 프롬프트에 사용한다. 단순 이미지 생성이나 영상의 내용 요약에는 적용하지 않는다.
---
...
- 정서적 단서: 표정, 눈꺼풀과 입술의 상태, 시선, 호흡과 신체 긴장, 피사체 사이 거리, 화면의 개방감·밀폐감, 빛과 재질이 주는 온도감, 정지 또는 흐름의 인상. 이미지에서 관찰되는 단서와 이를 바탕으로 해석한 감정을 구분한다.

렌즈 수치나 조명 장비명은 이미지에서 확인된 사실로 단정하지 않는다. 구현을 위한 선택이라면 그렇게 표시한다. 좌우가 불확실하면 인물의 해부학적 좌우보다 화면 기준 좌우와 보이는 귀·코·손 위치로 기술한다.

### MOOD & VISUAL 설계

스타일프레임이 있으면 숏을 쓰기 전에 `MOOD & VISUAL`을 설계한다. MOOD와 VISUAL은 별개의 장식 항목이 아니다. MOOD는 관객이 장면을 보며 느껴야 할 정서와 감각의 방향이고, VISUAL은 그 정서를 실제 화면과 소리에서 성립시키는 관찰 가능한 수단이다. `luxurious`, `cinematic`, `dreamy`, `emotional` 같은 형용사만 나열하지 않는다.

먼저 스타일프레임 전체에서 반복되는 정서적 단서를 근거로 한두 문장으로 핵심 무드를 정한다. 예를 들어 “고요한 자기 돌봄”을 선택했다면, 왜 그렇게 읽었는지 편안한 눈꺼풀, 느슨한 어깨, 투명한 전경층, 낮은 대비, 느린 빛의 변화처럼 이미지에서 확인되는 근거를 연결한다. 사용자의 컨셉이 참조와 다른 정서를 요구하면 사용자의 컨셉을 우선하고, 어떤 시각 요소를 유지하거나 조정할지 밝힌다.

그다음 핵심 무드를 다음 항목의 실행 가능한 표현으로 번역한다. 스타일프레임에 실제로 중요하거나 이번 영상에 필요한 항목만 사용한다.

- **관객 경험:** 친밀함, 긴장, 해방감, 경쾌함처럼 관객이 느낄 핵심 정서와 감각적 인상. 추상어를 한두 개의 명확한 방향으로 제한한다.
- **인물 퍼포먼스:** 표정, 시선, 눈꺼풀, 입술, 호흡, 자세, 신체 긴장, 손의 속도와 압력. “우아하게” 대신 무엇이 느리고 이완되어 보이는지 적는다.
- **공간과 레이어:** 공간의 개방감·밀폐감, 피사체와 배경의 거리, 전경 가림, 반사, 투명층, 소품과 여백이 정서에 기여하는 방식.
- **빛과 색:** 광원 방향, 확산 정도, 대비, 노출, 피부와 배경의 색온도 관계, 하이라이트와 그림자의 변화 속도. 색 이름만 나열하지 않는다.
- **재질과 광학:** 피부, 금속, 물, 유리, 천 등 핵심 재질의 표면감과 굴절·반사·플레어·보케가 감정에 주는 효과. 참조에 없는 효과를 분위기용으로 무분별하게 추가하지 않는다.
- **카메라와 시간감:** 프레이밍의 친밀도, 카메라 높이, 안정성, 이동 크기와 속도, 숏이 머무는 시간, 컷의 호흡. 빠른 편집과 고요한 무드처럼 충돌하는 선택은 의도된 경우에만 사용한다.
- **음향:** 음악의 밀도와 리듬, 환경음, 호흡, 재질음, 침묵이 화면의 감각을 어떻게 보완하는지. 이미지에서 음향을 관찰했다고 말하지 않고 연출 제안으로 구분한다.

스타일프레임이 여러 정서를 포함하면 하나로 억지로 평준화하지 않는다. 전체를 묶는 주된 정서와 숏을 따라 변화하는 감정의 흐름을 구분한다. 예: `낯선 긴장 → 호기심 → 안도`. 무드 진행이 없다면 일관된 감각을 유지한다고 명시한다.

최종 프롬프트에서는 참조 역할 다음에 `MOOD & VISUAL` 블록을 둔다. 첫 문단은 정서·인물 상태·시간감을 설명하고, 두 번째 문단은 공간·빛·색·재질·광학·카메라·음향이 그 정서를 어떻게 구현하는지 설명한다. 전역 규칙은 이 블록에서 한 번만 정하고, 각 숏에는 해당 숏에서 달라지는 구체적 표현만 적는다. 짧은 작업에서는 두 문단을 하나의 간결한 블록으로 합칠 수 있다.

권장 형태:

```text
MOOD & VISUAL
[관객이 느낄 정서와 인물의 내적 상태. 표정, 호흡, 자세와 영상의 시간감으로 표현한다.]
[그 정서를 구현하는 공간과 레이어, 빛과 색, 재질과 광학, 카메라와 편집 호흡, 음향을 서로 연결해 설명한다.]
```

핵심 무드는 인물 퍼포먼스·공간·빛과 색·재질과 광학·카메라와 시간감·음향 가운데 서로 다른 범주의 단서 두 가지 이상으로 구현한다. 세부적인 추상 표현에도 최소한 하나의 화면 또는 소리 단서를 연결한다. 항목을 기계적으로 모두 채우거나 시각 요소를 목록처럼 쌓지 말고, 선택한 요소가 같은 감정 방향을 만드는지 확인한다.
...
- 스타일프레임 또는 사용자 컨셉에서 읽은 핵심 MOOD와, 이를 표정·호흡·공간·빛·색·재질·카메라·편집·음향으로 구현하는 VISUAL 방향.
...
기본 구성은 제작 목표 → 참조와 인물 → MOOD & VISUAL → 시간순 숏과 전환 → 음향 → 연속성·필수 제외 조건이다. `MOOD & VISUAL`은 공간·빛·색·재질·광학·카메라·편집 호흡과 필요한 음향 방향을 포함한다. 숏별 동기화가 필요한 소리는 해당 숏에 다시 적는다. 짧은 영상에는 필요한 항목을 합친다. 플랫폼의 실제 설정은 프롬프트와 구분하고, 프롬프트 언어는 사용자 요청을 따른다. 대사는 지정된 언어와 문구를 보존한다.
...
- MOOD가 스타일프레임의 표정·자세·공간·빛·재질·시간감에서 읽은 근거와 연결되어 있는가? 브랜드용 형용사만 임의로 붙이지 않았는가?
- 핵심 무드가 서로 다른 범주의 관찰 가능한 수단 두 가지 이상으로 구현되고, 세부적인 추상 표현에도 화면 또는 소리 단서가 연결되었는가?
- VISUAL 요소가 같은 정서 방향을 만들며, 분위기와 무관한 효과를 장식처럼 나열하지 않았는가?
- 전체 무드와 숏별 감정 변화가 구분되어 있고 서로 충돌하지 않는가?
